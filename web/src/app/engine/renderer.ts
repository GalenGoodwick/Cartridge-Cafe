...
@fragment fn fs(in: VO) -> @location(0) vec4f {
  ...
  renderSuperimposed(
    ...
    for (let i = 0; i < fields.length; i++) {
      const f = fields[i]
      const off = i * 24
      // Defensive: a single malformed AI-built field (missing color / dims /
      // params) must NEVER crash this loop — one bad field would blank the whole
      // world (every other field packs after it and never gets written). Coerce.
      const psr = f.posScaleRot || [0, 0, 1, 0]
      const dims = f.shapeDims || [0, 0, 0, 0]
      const col = f.color || [1, 1, 1, 1]
      const vap = f.visualAndParams || [0, 0, 0, 0]
      const ext = f.extraParams || [0, 0, 0, 0]
      data[off +  0] = psr[0] ?? 0
      data[off +  1] = psr[1] ?? 0
      data[off +  2] = psr[2] ?? 1
      data[off +  3] = psr[3] ?? 0
