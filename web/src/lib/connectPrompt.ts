...
export const enterTheEye = (origin: string) =>
  `ENTER THE EYE — before you trust ANY build, and after every change, LOOK at real pixels on YOUR OWN GPU with the ready-made local eye. There is no bridge render_probe and no cloud eye; do NOT hand-roll a renderer or headless browser. Use the cartridge-cafe-mcp MCP — its \`render_probe\` renders IN-PROCESS on this machine's GPU (fast, private) — or \`npx -y --package=cartridge-cafe-mcp cartridge-cafe-eye\` (plain HTTP /render with the world snapshot from the bridge GET). Headless you are BLIND otherwise: a shader that fails to compile renders as NOTHING with no error reaching you, so you ship a black screen thinking it worked. Look after each change; fix the exact error line; never set brief_done until the eye shows what was asked. If you cannot get an eye at all, STOP and say so — never build blind.`
