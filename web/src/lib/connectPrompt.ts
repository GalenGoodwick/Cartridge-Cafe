...
export const enterTheEye = (origin: string) =>
  `ENTER THE EYE — before you trust ANY build, and after every change: probe the world (POST {"type":"render_probe"} to the bridge) and confirm a REAL render comes back — pixels, no WGSL compile errors, on-screen. Headless you are BLIND otherwise: a shader that fails to compile renders as NOTHING with no error reaching you, so you ship a black screen thinking it worked. Re-probe after each change; fix the exact error line; never set brief_done until the eye shows what was asked. If the eye stays closed (no render at all), STOP and say so — never build blind. PREFER THE READY-MADE LOCAL EYE — do NOT hand-roll your own renderer or headless browser: \`npx -y cartridge-cafe-mcp\` gives an MCP whose render_probe renders IN-PROCESS on this machine's own GPU (fast, private, no cloud), and \`npx -y --package=cartridge-cafe-mcp cartridge-cafe-eye\` starts the same eye as a plain HTTP service on localhost for any client (POST /render with the world snapshot from the bridge GET). There is NO cloud eye — render on your own GPU with one of these.`
