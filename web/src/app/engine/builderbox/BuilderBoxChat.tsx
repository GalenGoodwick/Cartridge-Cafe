...
export function BuilderBoxChat({ slotKey, channel, onFullChat, sendHumanShot, humanShot }: { slotKey: string; channel: string; onFullChat: () => void; sendHumanShot?: () => void; humanShot?: 'idle' | 'sending' | 'sent' | 'err' }) {
  ...
  return (
    <div className="border-t border-white/10 flex flex-col h-[280px]">
      <div className="flex items-center justify-between px-3 pt-1.5 font-mono text-[13px] tracking-[0.2em] text-white/45">
      ...
      <div className="px-3 pt-0.5 font-mono text-[12px] leading-snug text-white/70">
        CHAT = just talk: the maker and the room hear you; nothing is auto-summoned.
      </div>
      <div ref={scrollRef} onScroll={checkBottom} className="flex-1 min-h-0 overflow-y-auto px-3 py-1 font-mono text-[14px] leading-relaxed">
        {msgs.length === 0
          ? <div className="text-white/70">say something — the maker and the room hear it.</div>
          : msgs.slice(-40).map((m, i) => (
            <div key={m.at + '-' + i} className="text-white/75">
              ...
              <span className="text-white/70"> · </span>{m.text}
            </div>
          ))}
      </div>
      {postErr && <div className="px-3 pb-1 font-mono text-[13px] text-red-400/90">{postErr}</div>}
      <div className="flex gap-1.5 px-2 pb-2">
        <input value={draft} onChange={e => setDraft(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') void say() }}
          ...
          className="flex-1 bg-white/5 border border-white/10 rounded-md px-2.5 py-1.5 font-mono text-[14px] text-white/85 placeholder:text-white/65 outline-none focus:border-white/30" />
        <button onClick={() => void say()} className="px-3 rounded-md bg-white/10 hover:bg-white/20 font-mono text-[14px] text-white/70">➤</button>
      </div>
    </div>
  )
}
