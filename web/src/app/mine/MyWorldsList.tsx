    ...
    <div className="min-h-screen px-4 py-8 font-mono" style={{ background: 'radial-gradient(120% 90% at 50% 0%, #0c0b14, #050509)', color: '#e7dcc8' }}>
      <div className="max-w-[720px] mx-auto">
        <div className="flex items-baseline gap-3 mb-1">
          ...
          <span className="text-white/45 text-[14px]">{live.length}</span>
          <Link href={`/u/${handle}`} className="ml-auto text-[13px] tracking-[0.15em] text-cyan-200/70 hover:text-cyan-200">MY PUBLIC SHELF ↗</Link>
        </div>
        <p className="text-white/50 text-[14px] mb-6">the workbench — open, inspect, delete. co-built (open) worlds are protected.</p>

        {live.length === 0 && <div className="text-white/50 text-[14px] py-12 text-center">no worlds yet — brew one from the main page.</div>}
          ...
          {live.map(w => (
            <div key={w.slug} className="flex items-center gap-3 px-3 py-2.5 rounded-xl border border-white/10 bg-black/40">
              <Link href={`/space/${w.slug}`} className="min-w-0 flex-1 group">
                ...
                <span className="ml-2 text-[13px] text-white/40">/{w.slug}</span>
              </Link>
              <span className="text-[12px] tracking-[0.1em] text-white/45">{w.fit === 'universal' ? '◇ universal' : w.fit === 'mobile' ? '▯ mobile' : '🖥 desktop'}</span>
              {w.forks > 0 && <span className="text-[12px] text-emerald-200/60">⑂{w.forks}</span>}
              <span className={`text-[12px] tracking-[0.1em] ${w.isPublic ? 'text-amber-200/70' : 'text-white/40'}`}>{w.isPublic ? '● playable' : '○ private'}</span>
              {w.open ? (
                <span className="text-[12px] tracking-[0.1em] text-cyan-200/60" title="OPEN world — members may have built here; co-built work is protected. Close it (make solo) to unlock deletion.">🔒 open · protected</span>
              ) : (
                <button onClick={() => { setConfirm(w); setTyped(''); setErr('') }}
                  className="text-[13px] px-2 py-1 rounded-lg border border-red-400/25 text-red-300/60 hover:text-red-300 hover:bg-red-500/10">✕</button>
              )}
            </div>
          ))}
        </div>
      ...
      {confirm && (
        <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/70 p-6" onClick={() => setConfirm(null)}>
          <div className="max-w-sm w-full rounded-xl border border-red-400/30 bg-black/95 p-5" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-white/55 text-[13px] mb-1">type the world&apos;s name to confirm:</p>
            ...
            <div className="flex justify-end gap-2">
              <button className="px-3 py-1.5 rounded-lg border border-white/20 text-white/70 hover:bg-white/10 text-[14px]" onClick={() => setConfirm(null)}>KEEP IT</button>
              <button disabled={typed.trim() !== confirm.name || busy} onClick={() => doDelete(confirm)}
                className="px-3 py-1.5 rounded-lg border border-red-400/50 bg-red-500/20 text-red-200 hover:bg-red-500/30 text-[14px] disabled:opacity-35">
                {busy ? '…' : 'DELETE'}
              </button>
            </div>
          </div>
