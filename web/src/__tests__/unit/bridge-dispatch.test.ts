...
describe('bridge dispatch', () => {
  it('unmigrated verbs return null → legacy chain handles them', async () => {
    expect(await dispatchBridgeCommand({ cmd: { type: 'create_field' }, auth: noSpace, origin: 'http://t', tokenHolder: 'h' })).toBeNull()
    expect(await dispatchBridgeCommand({ cmd: { type: 'no_such_verb' }, auth: noSpace, origin: 'http://t', tokenHolder: 'h' })).toBeNull()
  })
  ...
  it('a space-scoped verb without a world token is refused at the table, not in the handler', async () => {
    const r = await dispatchBridgeCommand({ cmd: { type: 'build_status' }, auth: noSpace, origin: 'http://t', tokenHolder: 'h' })
    expect(r).toBeTruthy()
