...
export function NotifyMeButton({
  ...
  if (permission === 'denied' && !isSubscribed) {
    return (
      <p className={`font-mono text-[14px] text-grounds/70 ${variant === 'menu' ? 'px-3 py-2' : 'text-center'}`}>
  ...
  if (variant === 'menu') {
    return (
      <button onClick={onClick} disabled={isLoading}
        ...
        {err && <span className="block text-[14px] text-flame/70 tracking-normal mt-0.5">{err}</span>}
      </button>
    )
  }
  ...
  return (
    <div className={`flex flex-col items-center gap-1 ${className}`}>
      ...
      {err && <span className="font-mono text-[14px] text-red-300/80">{err}</span>}
    </div>
  )
}
