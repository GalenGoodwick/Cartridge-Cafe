...
Your view is yours: it never takes my seat and never counts in head-counts.`
  ...
  // the door shader can pip the worlds you haven't entered yet. Signed-out guests
  // get it too (localStorage, like cc-pid). Published to window.__cafeVisited; the
  // door hook reads it every frame. markVisited mutates the SAME object, so the
  // pip clears live without republishing.
  ...
  useEffect(() => {
    ...
    const onLaunch = async (e: Event) => {
      ...
      // Only real worlds count (space:<slug> or a bare scene name), not the
      // sub/maker/players/house nav bubbles.
        ...
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-void/80 backdrop-blur-sm"
          ...
          <div className="relative w-[480px] max-w-[94vw] border border-brass/40 rounded-xl px-7 py-6 bg-void/95 shadow-[0_0_60px_rgba(245,176,76,0.15)]"
            ...
            <div className={'transition-opacity ' + (connectReady ? 'opacity-100' : 'opacity-35 pointer-events-none select-none')}>
              ...
              {/* HOUSE-AI build button backlogged: the headless builder can't drive
                  the engine like a real terminal yet, so the auto-build path is
                  removed. finalizeBrief / houseAiUp / the /api/builds routes stay
                  in place for when it's reliable. Manual COPY-PROMPT is the path. */}
                ...
                {acctOpen && (
                  <div className="absolute right-0 top-full mt-2 w-52 rounded-xl bg-[#171009]/95 backdrop-blur border border-[#b97a2a]/25 p-1.5 z-[70] font-mono text-[14px]">
                    {[
                      ...
                      // LEND AI backlogged: the volunteer-swarm builder isn't reliable
                      // enough yet (headless agent can't drive the engine like a terminal).
                      // LendAiPanel + /api/builds/enroll stay for when it is.
                    ].map(it => (
