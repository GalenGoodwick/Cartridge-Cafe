...
async function tick() {
  ...
  try {
    ...
    const prompt = [
      ...
      `   COORDINATE SPACE: the world is a 512×512 grid, camera fixed at the CENTER (256,256). Build AROUND (256,256), never around (0,0), and NEVER use negative x/y — a world centered on origin renders off-screen in the corner (looks dark). A field's x,y is its center.`,
      `   FIELD IS A CANVAS, the shader's ALPHA is the shape. A skinned create_field (one with a visualType) defaults to a FULL-SCREEN field — the visual paints the whole view and its own alpha carves what's visible; draw moving things INSIDE it via the uni() whiteboard from your step hook (do NOT make a tiny field per entity). Only pass an explicit w/h (a movable box) or radius (a disc) when you truly want a small BOUNDED field that translates on its own. Never rely on the old default — a bare skinned field used to be a 20px dot.`,
      ...
      `6b. VERIFY IT PLAYS — a world can RENDER perfectly and still ignore every control (a "rhythm game" whose character never moves). If the brief promises ANY interactivity (move, jump, steer, press, drag), cafe_probe with input:"auto" (or "run-right" for a platformer/runner, "tap-action" for press-timing, "sweep-cursor" for a cursor/aim world). It presses the controls and returns inputReport.respondsToInput. If that is FALSE, your control wiring is broken — the hook must read wd.input (moveX/moveY/action/actionHeld/pointer) or the raw wd.key_*/wd.mouse_x/wd.mouse_y — fix it and re-probe until it's TRUE. Do NOT ship an unplayable game that merely looks right.`,
      `7. Only when cafe_probe confirms the world RENDERS the brief (and, if interactive, respondsToInput is TRUE), cafe_send set_world_data {"data":{"brief_done":true}}. The bridge also refuses brief_done while no field has a registered visualType.`,
      ``,
