...
export const COMMAND_REGISTRY: Record<string, CommandSpec> = {
  ...
  remove_step_hook: { scope: 'space', group: G.nodes, desc: 'remove a hook node (holder/owner)', params: { hookId: 'the node', force: 'override a base manifest LOAD-BEARING refusal?' } },
  claim_node: {
  ...
  remove_module: { scope: 'space', group: G.visuals, enforce: true, desc: 'remove a module', params: { name: 'module name', force: 'override a base manifest LOAD-BEARING refusal?' } },
  ...
  delete_field: { scope: 'space', group: G.fields, enforce: true, desc: 'remove a field', params: { fieldId: 'the field', force: 'override a base manifest LOAD-BEARING refusal?' } },
