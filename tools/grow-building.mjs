...
const shiftZ = (e, zb) => {
  ...
  if (e.c) o.c = mv(e.c)
  if (e.zc !== undefined) o.zc = e.zc + zb
  return o
}
...
export function sdElement(e, p) {
  ...
  if (e.kind === 'lancetrim') {
    const op = sdOpening2D(q[2] - e.zc, q[1], e.hw, e.springY, e.k, e.R)
    const fc = Math.max(Math.abs(q[0] - e.X) - e.faceHalf, 0)
    // floor clamp: the open-bottom profile would run the roll below ground
    return Math.max(Math.hypot(op, fc) - e.r, -q[1] - 0.05)
  }
  if (e.kind === 'lancetcut') {
    const op = sdOpening2D(q[2] - e.zc, q[1], e.hw, e.springY, e.k, e.R)
    return Math.max(op, Math.abs(q[0] - e.X) - e.cutHalf)
  }
...
let emitN = 0
function emitEl(e, pv, dv, zvar, helper) {
  ...
  let call
  if (e.kind === 'strut') call = `GPRIM_strut(${P}, ${mv(e.a)}, ${mv(e.b)}, ${f(e.r1)}, ${f(e.r2)})`
  else if (e.kind === 'bez') call = `GPRIM_bez(${P}, ${mv(e.a)}, ${mv(e.ctl)}, ${mv(e.b)}, ${f(e.r)})`
  else if (e.kind === 'box') call = `GPRIM_box(${P} - ${zvar ? `vec3f(${f(e.c[0])}, ${f(e.c[1])}, ${zvar} + ${f(e.c[2])})` : v3(e.c)}, ${v3(e.h)})`
  else if (e.kind === 'lancetrim') {
    const n = ++emitN
    const op = `${helper}(${pv}.z - ${zAt(e.zc)}, ${pv}.y, ${f(e.hw)}, ${f(e.springY)}, ${f(e.k)}, ${f(e.R)})`
    const lines = [
      `let go${n} = ${op};`,
      `let gf${n} = max(abs(${px} - ${f(e.X)}) - ${f(e.faceHalf)}, 0.0);`,
    ]
    call = `max(sqrt(go${n} * go${n} + gf${n} * gf${n}) - ${f(e.r)}, -${pv}.y - 0.05)`
    const t = e.tissue || 0
    ...
    const op = `${helper}(${pv}.z - ${zAt(e.zc)}, ${pv}.y, ${f(e.hw)}, ${f(e.springY)}, ${f(e.k)}, ${f(e.R)})`
    return `${dv} = max(${dv}, -max(${op}, abs(${px} - ${f(e.X)}) - ${f(e.cutHalf)}));`
  }
  const t = e.tissue || 0
...
export function emitWGSL(graph, name, prims) {
  emitN = 0
  const L = []
  ...
  L.push(`  if (p.x < ${f(bl[0])} || p.x > ${f(bh[0])} || p.y > ${f(bh[1])} || p.z < ${f(bl[2])} || p.z > ${f(bh[2])}) { return 1e5; }`)
  L.push(`  var d = 1e5;`)
  for (const e of graph.statics) L.push('  ' + emitEl(e, 'p', 'd', null, helper))
  for (const rp of graph.repeats) {
    L.push(`  {`)
    L.push(`    let ci = clamp(floor((p.z - ${f(rp.origin)}) / ${f(rp.cellW)}), 0.0, ${f(rp.count - 1)});`)
    L.push(`    let zb = ${f(rp.origin)} + ci * ${f(rp.cellW)};`)
    L.push(`    var s = 1e5;`)
    for (const e of rp.elements) L.push('    ' + emitEl(e, 'p', 's', 'zb', helper))
    ...
    L.push(`    if (nOk) {`)
    for (const e of rp.elements) L.push('      ' + emitEl(e, 'p', 's', 'zn', helper))
    L.push(`    }`)
    if (rp.rhythm.length) {
      ...
      for (const e of rp.rhythm) L.push('      ' + emitEl(e, 'p', 's', 'zr', helper))
      L.push(`    }`)
    }
    // opening cuts LAST, for both visited cells (max is idempotent)
