...
// No frag/vert/comp/frag/vert/comp/frag/vert/comp. No builtin redeclarations.
...
fn visual_aureon(uv: vec2f, sdf: f32, color: vec4f, time: f32, params: vec4f, behind: vec4f) -> vec4f {
  ...
  var jhit = -1.0;
  let jbC = vec3f(0.6, 1.8, 0.5); let jbR = 7.0;
  let jproj = dot(jbC - ro, rd);
  let jclose = length(ro + rd * max(jproj, 0.0) - jbC);
  if (jclose < jbR && jproj > -jbR && (jproj - jbR) < maxJT) {
    var jt = max(jproj - jbR, 0.02);
    for (var i = 0; i < 80; i = i + 1) {
      let jp = ro + rd * jt;
      let d = au_jelly(jp, t);
      if (d < 0.001 * jt + 0.0006) { jhit = jt; break; }
      jt += d * 0.9;
      if (jt > maxJT || jt > jproj + jbR) { break; }
    }
  }
  if (jhit > 0.0) {
    let jp = ro + rd * jhit;
