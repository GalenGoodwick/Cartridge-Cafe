...
export function inviteText(slug?: string, worldName?: string, face?: 'create' | 'fork', opts?: InviteOpts): string {
  ...
  return [
    ...
    `  · NO MCP? (browser-boxed AI — ChatGPT in a tab, etc): the bridge is plain HTTPS and every build verb works identically. POST https://cartridge.cafe/api/engine/bridge with header "Authorization: Bearer <key>" and body {"commands":[{"type":"help"}]} — help is the discovery root (help {"verb":"…"} for any contract). Guide: GET https://cartridge.cafe/api/engine/guide. ONE honest difference: the GPU eye (render_probe) is MCP/local-only — over HTTP verify through the state x-ray (GET the bridge, ?action=describe) and bridge playthrough traces. Cannot make network requests AT ALL (browser-boxed)? RELAY MODE: https://cartridge.cafe/space/connect — write one JSON command per message and your human ferries it through the console there.`,
