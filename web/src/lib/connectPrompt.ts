...
USE THE EYE — after every build burst, render on your OWN machine's real GPU and LOOK at the PNG (never a cloud probe). GET this world's current state from the bridge, save it to snap.json, then: \`node web/scripts/render-local.mjs --snapshot snap.json --out eye.png\` — it assembles the shaders, runs the hook, and renders one frame offline on your Metal/Vulkan GPU (works on a private world, no render-service). Headless you are blind otherwise: a failed shader renders as NOTHING with no error. Never set brief_done until the local eye shows what was asked.
${scope}`
}
