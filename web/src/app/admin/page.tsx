...
const chip = 'font-mono text-[11px] tracking-[0.18em] px-2.5 py-1 rounded-lg border transition-colors'
...
export default function AdminPage() {
  ...
  const Row = ({ r, sub }: { r: Root; sub?: boolean }) => (
    <div className={`${box} flex items-center gap-3 px-3.5 py-2.5 mb-1.5 ${r.private ? 'opacity-60' : ''} ${sub ? 'ml-7' : ''}`}>
      <div className="flex-1 min-w-0 font-mono">
        <div className="text-[14px] text-white/90 truncate">{r.name}</div>
      ...
      {r.branches.length > 0 && (
        <button onClick={() => setOpenRoot(openRoot === r.name ? '' : r.name)}
          className={`${chip} border-white/15 bg-black/40 text-white/70 hover:text-white`}>
      ...
      {busy === (r.space ?? r.name) ? <span className="font-mono text-[12px] text-white/70">…</span> : <>
  ...
  const BranchRow = ({ b }: { b: Branch }) => (
    <div className={`${box} flex items-center gap-3 px-3.5 py-2 mb-1.5 ml-7 ${b.private ? 'opacity-60' : ''}`}>
      <div className="flex-1 min-w-0 font-mono text-[13px] text-white/75 truncate">
        ⑂ {b.label} <span className="text-white/45 text-[11px]">· {b.versions} version{b.versions > 1 ? 's' : ''}</span>
      </div>
      ...
      <a href={`/grid?w=${encodeURIComponent(b.latest)}&ui=games&ph=play`} target="_blank" rel="noreferrer"
        className={`${chip} border-amber-300/40 bg-amber-400/10 text-amber-200 hover:bg-amber-400/20`}>VIEW</a>
      {busy === b.base ? <span className="font-mono text-[12px] text-white/70">…</span> : <>
  ...
  const Stat = ({ label, value }: { label: string; value: number }) => (
    <div className={`${box} px-3.5 py-2 min-w-[96px]`}>
      ...
      <div className="font-mono text-[10px] tracking-[0.18em] text-white/50">{label}</div>
    </div>
  )
  ...
  const Overview = () => {
    ...
    return (
      <section className={`${box} p-4 mb-4`}>
        <div className="font-mono text-[11.5px] tracking-[0.2em] text-amber-200/80 mb-3">AT A GLANCE</div>
        ...
        {a?.allTime?.since && (
          <div className="font-mono text-[10.5px] text-white/45 mb-4">
            since {new Date(a.allTime.since).toLocaleDateString()} · VISITORS ≤ counts distinct daily ids — an upper bound, not a head-count
          </div>
        )}
        {refs.length > 0 && (
          <>
            <div className="font-mono text-[10.5px] tracking-[0.18em] text-white/50 mb-1.5">WHERE THEY COME FROM · {win}</div>
            <div className="mb-4">
              {refs.map(r => (
                <div key={r.source} className="flex gap-2.5 py-1 border-b border-white/5 font-mono text-[12.5px]">
                  <span className={`flex-1 truncate ${r.source === '(direct)' ? 'text-white/55' : 'text-white/75'}`}>{r.source}</span>
                  <span className="text-white/70 w-24 text-right">{r.visitors.toLocaleString()} visitors</span>
                  <span className="text-white/45 w-16 text-right">{r.hits.toLocaleString()} hits</span>
                </div>
              ))}
            </div>
          </>
        ...
        {worldRows.length > 0 && (
          <>
            <div className="font-mono text-[10.5px] tracking-[0.18em] text-white/50 mb-1.5">WORLDS · {win} · newcomers &amp; how long they stay</div>
            {worldRows.map(w => (
              <div key={w.label} className="flex items-center gap-2.5 py-1 border-b border-white/5 font-mono text-[12.5px]">
                ...
                <span className="text-white/70 w-14 text-right">{w.visitors ?? '·'} vis</span>
  ...
  const BridgeWatch = () => {
    ...
    return (
      <section className={`${box} p-4 mb-4`}>
        <div className="font-mono text-[11.5px] tracking-[0.2em] text-amber-200/80 mb-3">TRAFFIC &amp; BRIDGE WATCH · LAST 48H</div>
        ...
        <div className="font-mono text-[10.5px] tracking-[0.18em] text-white/50 mb-1">BRIDGE HITS / HOUR (peak {peak.toLocaleString()})</div>
        <div className="flex items-end gap-[2px] h-12 mb-4">
          {bridgePerHour.length === 0 && <div className="font-mono text-[12px] text-white/50">no bridge traffic in the window</div>}
          {bridgePerHour.map(h => (
        ...
        <div className="font-mono text-[10.5px] tracking-[0.18em] text-white/50 mb-1.5">TOP TALKERS · BY TOKEN · LAST 24H</div>
        {topTalkers.length === 0 && <div className="font-mono text-[12px] text-white/50">quiet — no agents in the last 24h</div>}
        {topTalkers.map(t => {
          const hot = t.hits >= HOT_TALKER && !t.who.startsWith('house')
          return (
            <div key={t.who} className="flex items-center gap-2.5 py-1 border-b border-white/5 font-mono text-[12.5px]">
              <span className={`w-36 truncate ${hot ? 'text-amber-300' : 'text-white/75'}`}>{hot && '⚠ '}{t.who}</span>
              <span className={`flex-1 ${hot ? 'text-amber-200' : 'text-white/70'}`}>{t.hits.toLocaleString()} hits</span>
              <span className="text-white/45 text-[11px]">last {new Date(t.last).toLocaleTimeString()}</span>
            </div>
          )
        })}
  ...
  const Companies = () => (
    <section className={`${box} p-4 mb-4`}>
      <div className="font-mono text-[11.5px] tracking-[0.2em] text-amber-200/80 mb-1">◆ COMPANIES · PROPRIETARY TENANTS</div>
      <p className="font-mono text-[10.5px] text-white/50 mb-3">provision a white-label customer: bind a chosen handle to their account + grant IP control. they get /c/&lt;handle&gt; (and &lt;handle&gt;.cartridge.cafe once DNS is pointed). bill by Stripe invoice, net-30.</p>
      <div className="grid grid-cols-2 gap-2 mb-2">
        <input value={coForm.email} onChange={e => setCoForm({ ...coForm, email: e.target.value })} placeholder="owner email (must have signed in once)"
          className="font-mono text-[13px] bg-black/50 border border-white/15 rounded-lg px-3 py-2 text-white/85 outline-none focus:border-amber-300/50" />
      ...
      {coNote && <div className="font-mono text-[12px] text-amber-200/90 mt-2">{coNote}</div>}
      {companies && companies.length > 0 && (
        <div className="mt-4 flex flex-col gap-1.5">
          {companies.map(c => (
            <div key={c.handle} className="flex items-center gap-2 py-1.5 border-b border-white/5 font-mono text-[12.5px]">
              <a href={`/c/${c.handle}`} className={`w-28 truncate ${c.ipControl ? 'text-amber-100' : 'text-white/50'}`}>◆ {c.handle}</a>
              <span className="flex-1 truncate text-white/65">{c.ownerEmail} · {c.worlds} world{c.worlds === 1 ? '' : 's'}{c.domain ? ' · ' + c.domain : ''}{!c.ipControl && ' · ⚠ NO IP CTRL'}</span>
              <button onClick={() => invoice(c.handle, c.name)} disabled={busy === 'inv:' + c.handle}
                className="text-[11px] tracking-[0.12em] px-2 py-1 rounded border border-white/20 text-white/70 hover:bg-white/10 disabled:opacity-40">
                {busy === 'inv:' + c.handle ? '…' : 'INVOICE'}
              </button>
              <button onClick={() => deprovision(c.handle)} disabled={busy === 'deprov:' + c.handle}
                className="text-[11px] tracking-[0.12em] px-2 py-1 rounded border border-red-400/30 text-red-200/80 hover:bg-red-500/15 disabled:opacity-40">
      ...
      {companies && companies.length === 0 && <div className="font-mono text-[12px] text-white/45 mt-3">no companies provisioned yet.</div>}
    </section>
  )
    ...
    <div className="min-h-screen px-4 py-8" style={{ background: 'radial-gradient(120% 90% at 50% 0%, #0c0b14, #050509)' }}>
      <div className="max-w-[860px] mx-auto">
        <div className="flex items-center gap-3 mb-1">
          <a href="/grid" className="font-mono text-[14px] w-9 h-9 grid place-items-center rounded-xl border bg-black/60 border-white/20 text-white/75 hover:text-white transition-colors">◂</a>
        ...
        {!err && !roots && <div className="font-mono text-[13px] text-white/60">fetching the shelf…</div>}

        {roots && (
          <>
            {/* ── WORLDS — the real shelf (player spaces) ── */}
            <div className="flex items-center gap-3 mb-2 mt-2">
              <div className="font-mono text-[11.5px] tracking-[0.2em] text-amber-200/80 shrink-0">WORLDS · {spaces.length}</div>
              <input value={q} onChange={e => setQ(e.target.value)} placeholder="filter worlds, scenes, makers…"
                className="flex-1 font-mono text-[12.5px] px-3 py-1.5 rounded-lg bg-black/50 border border-white/15 text-white/85 placeholder:text-white/40 outline-none focus:border-amber-300/50" />
            </div>
            {spaces.map(r => <Row key={'space:' + r.space} r={r} />)}
            {spaces.length === 0 && <div className="font-mono text-[12px] text-white/50 mb-3">no worlds match.</div>}
            ...
            <button onClick={() => setLibOpen(o => !o)}
              ...
              <span className="font-mono text-[11.5px] tracking-[0.2em] text-white/70">{libOpen ? '▾' : '▸'} THE SCENE LIBRARY · {scenes.length}</span>
              <span className="font-mono text-[11px] text-white/45">legacy store scenes — bases, tests, the pre-space era. A library, not the shelf.</span>
            </button>
        ...
        {!err && (
          <div className="mt-10">
            <div className="flex items-baseline gap-3 mb-1">
              <div className="font-mono text-[14px] tracking-[0.2em] text-red-200/90 flex-1">FAULTS FROM THE FIELD</div>
              <button onClick={loadFaults} className={`${chip} border-white/15 bg-black/40 text-white/70 hover:text-white`}>↻ REFRESH</button>
            </div>
            <p className="font-mono text-[11.5px] text-white/50 mb-3">errors from real sessions, newest first — source-documented where the engine holds the source.</p>
            {faults && faults.length > 0 && (
              <input value={faultFilter} onChange={e => setFaultFilter(e.target.value)} placeholder="filter by scene, phase, hook, message…"
                className="w-full mb-3 font-mono text-[12.5px] px-3 py-2 rounded-lg bg-black/50 border border-white/15 text-white/85 placeholder:text-white/40 outline-none focus:border-amber-300/50" />
            )}
            {!faults && <div className="font-mono text-[13px] text-white/60">reading the log…</div>}
            {faults && faults.length === 0 && <div className="font-mono text-[13px] text-emerald-300/90">no faults logged — clean skies.</div>}
              ...
              return (
                <div key={i} className={`${box} px-3.5 py-2.5 mb-2 font-mono`}>
                  <div className="flex items-center gap-2.5 flex-wrap">
                    <span className={`text-[10px] tracking-[0.15em] text-black font-bold rounded px-1.5 py-0.5 ${tone}`}>{p.toUpperCase()}</span>
                    {f.scene && <span className="text-[13px] text-amber-100/90">{f.scene}</span>}
                    <span className="flex-1" />
                    <span className="text-[11px] text-white/50">{new Date(f.at).toLocaleString()}</span>
                  </div>
                  {f.url && <div className="text-[11px] text-white/45 mt-1 truncate">{f.url}</div>}
                  {f.hazards.map((h, j) => (
                    <div key={j} className="mt-2">
                      <div className="text-[13px] text-red-100/90">
                        {h.name && <span className="text-red-200 font-bold">{h.name}</span>}
                        {h.author && <span className="text-emerald-300/90 text-[11px]"> · by {h.author}</span>}
                        {typeof h.line === 'number' && h.line > 0 && <span className="text-white/55 text-[11px]"> · line {h.line}{typeof h.col === 'number' ? ':' + h.col : ''}</span>}
                        {h.gpuModel && <span className="text-white/55 text-[11px]"> · {h.gpuModel}</span>}
                      </div>
                      {h.reason && <div className="text-[12px] text-white/75 mt-0.5 whitespace-pre-wrap break-words">{h.reason}</div>}
                      {h.snippet && <pre className="text-[12px] text-white/70 bg-black/60 border border-white/10 rounded-lg px-2.5 py-2 mt-1.5 overflow-x-auto">{h.snippet}</pre>}
                      {!h.snippet && h.stack && <pre className="text-[11px] text-white/55 bg-black/50 border border-white/8 rounded-lg px-2.5 py-2 mt-1.5 overflow-x-auto whitespace-pre-wrap">{h.stack}</pre>}
                    </div>
                  ))}
                </div>
              )
        ...
        <div className="mt-7 font-mono text-[10.5px] tracking-[0.3em] text-amber-200/30">CARTRIDGE.CAFE · KEEPER ONLY</div>
      </div>
    </div>
  )
}
