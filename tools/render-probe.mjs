...
//
// It TICKS the world's step hooks first (like web/src/app/engine/world-sandbox.ts)
// so hook-driven worlds render live, not their dead boot state, and every hook
// throw surfaces as hookErrors. It renders SEVERAL frames across the ticks and
// derives a MOTION profile (travel, frame-delta, vibration, divergence, settling)
// from the struct series — so "everything vibrates" / "the sim diverges" become
// measured facts, not guesses.
//
// Output: a pixel-state STRUCT (brightness, coverage, bbox, dominant colors,
// quadrants) + hookErrors + motion + a PNG for a vision agent to cross-confirm.
// Struct and image come from the SAME buffer, so they must agree.
//
// Deno: deno run -A --unstable-webgpu tools/render-probe.mjs --state s.json --name v --out o.png [--ticks 45] [--samples 6]
...
const NTICKS = A.ticks !== undefined ? parseInt(A.ticks) : 45;
const DT = 1 / 60;
...
// advance the sim by one tick (all hooks), collecting throws once per hook
function tickOnce(t) {
...
// ── compose shader (only the target visual + shared modules; engine isolates each) ──
const wgsl = `
...
// render one frame at (time), with the current gpuUniforms + ticked field transform,
// read it back, and compute its per-frame stats
...
const NSAMPLES = (NTICKS > 0 && compiled.length) ? Math.max(2, parseInt(A.samples || "6")) : 1;
...
for (const target of sampleTicks) {
  ...
  samples.push(await sample((A.time !== undefined ? parseFloat(A.time) : cur * DT)));
}
...
if (samples.length >= 2) {
  ...
  motion = {
    ...
    moving: avgDelta > 1.5,
    vibrating: avgDelta > 4 && travelX < 0.02 && travelY < 0.02,   // lots of change, no travel = jitter not locomotion
    diverging: nan || up(lums) || up(covs),
    settling: deltas.length >= 3 && mean(deltas.slice(-2)) < mean(deltas.slice(0, 2)) * 0.6,
  };
}

// ── output: final (most-evolved) frame's struct + motion + png ──
const last = samples[samples.length - 1];
const out = A.out || "/tmp/render-probe.png";
await Deno.writeFile(out, encode({ width: S, height: S, data: last.data, channels: 4 }));
