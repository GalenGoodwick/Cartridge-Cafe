      ...
      <div className="relative z-10 w-full max-w-sm">
        ...
        <div className="cart cafe-steam arrive">
          ...
          <div className="px-6 py-6 space-y-3">
            ...
            {session?.user?.isTemp && (
              <p className="font-mono text-[10px] leading-relaxed text-brass text-center pb-1">
            ...
            {session?.user && (
              <div className="rounded-lg border border-brass/25 bg-void/30 px-4 py-3.5 space-y-2">
                <div className="font-mono text-[10px] tracking-[0.25em] text-brass">YOU ARE IN AS {(session.user.email || '').toUpperCase()}</div>
                <p className="font-sans text-[11px] text-grounds">secure this device — next time, your face or fingerprint is the key.</p>
                <button
                  onClick={async () => {
                    setEnrolled('')
                    try {
                  ...
                  className="w-full rounded-lg border border-flame/40 hover:border-flame/70 text-flame/90 hover:text-glow font-mono text-[11px] tracking-[0.2em] px-6 py-3 transition-all"
                >
                  ⌘ ADD FACE ID / TOUCH ID
                </button>
                {enrolled && <p className="font-mono text-[10px] text-grounds">{enrolled}</p>}
              </div>
            )}
            {errorCode && (
              <p className="font-mono text-[10px] leading-relaxed text-flame/90 text-center pb-1">
            ...
            {providers && !!providers.guest && !session?.user && (
              <button
                ...
                className="w-full rounded-lg border border-dashed border-brass/40 hover:border-flame/60 text-grounds hover:text-steamer font-mono text-[11px] tracking-[0.2em] px-6 py-3 transition-all"
              >
