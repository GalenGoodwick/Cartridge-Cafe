      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-full mt-1 z-50 w-[220px] max-h-[260px] overflow-y-auto rounded-lg bg-[#0d0906]/95 backdrop-blur border border-white/15 shadow-xl py-1">
            <div className="px-3 py-1.5 text-[9px] tracking-[0.25em] text-white/40 border-b border-white/10">VERSIONS · {total}</div>
            {items.map(it => (
              <button
                key={it.key}
                onClick={() => { it.onPick(); setOpen(false) }}
                className={`w-full flex items-baseline justify-between gap-2 px-3 py-1.5 text-left text-[10px] font-mono transition-colors ${
              ...
              >
                ...
                {it.sub && <span className="text-white/30 text-[9px] truncate">{it.sub}</span>}
              </button>
            ))}
          </div>
        </>
