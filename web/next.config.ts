    ...
    return [
      {
        ...
        headers: [
          { key: 'X-Frame-Options', value: 'SAMEORIGIN' },   // the grid embeds /create (same-origin only; cross-site framing stays blocked)
          { key: 'X-Content-Type-Options', value: 'nosniff' },
          { key: 'Referrer-Policy', value: 'strict-origin-when-cross-origin' },
          { key: 'Strict-Transport-Security', value: 'max-age=63072000; includeSubDomains' },
          { key: 'Permissions-Policy', value: 'camera=(), microphone=(), geolocation=(), pointer-lock=(self)' },
          { key: 'Content-Security-Policy', value: "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval' https://challenges.cloudflare.com; worker-src 'self' blob:; style-src 'self' 'unsafe-inline'; img-src 'self' data: https: blob:; font-src 'self' https://fonts.gstatic.com; connect-src 'self' https://*.vercel.app https://challenges.cloudflare.com https://*.up.railway.app wss://*.up.railway.app http://localhost:8080 ws://localhost:8080 http://localhost:3334; frame-src 'self' https://challenges.cloudflare.com; frame-ancestors 'self'; object-src 'none'; base-uri 'self'" },
        ],
      },
      {
